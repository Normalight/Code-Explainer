...
import { getFileTree, getProgress, getStructure, getDependencies, getExportUrl, importGitHub, getCommits, getCommitDiff, reviewCommit, getChatSessions, deleteChatSession, listProjects } from '../api';
    ...
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg)' }}>
      ...
      {!sidebarCollapsed && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 8px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            <span
              onClick={() => navigate('/projects')}
              style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', color: 'var(--text)', opacity: 0.5 }}
            ...
            <span
              ...
              style={{
                marginLeft: 'auto', padding: '2px 6px', fontSize: 11, color: 'var(--text)',
            ...
            <span
              onClick={() => setShowSettings(true)}
              style={{
                padding: '2px 6px', fontSize: 11, color: 'var(--text)',
                background: 'var(--code-bg)', border: '1px solid var(--border)', borderRadius: 4,
      ...
      <div
        ...
        style={{
          width: sidebarCollapsed ? 24 : 4,
          cursor: 'col-resize',
          background: sidebarCollapsed ? 'var(--border)' : 'transparent',
          display: 'flex',
          alignItems: 'center',
        ...
        onMouseLeave={(e) => { e.currentTarget.style.background = sidebarCollapsed ? 'var(--border)' : 'transparent'; }}
              ...
              {progress && (
                <div style={{ marginBottom: 32 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    <h2 style={{ fontSize: 20, fontWeight: 600 }}>{t('analysisProgress')}</h2>
                    <span style={{ color: 'var(--text)', fontSize: 13 }}>
                      {progress.completed}/{progress.total} {t('sourceFiles')}
                      {progress.skippedFiles > 0 && (
                        <span style={{ opacity: 0.5, marginLeft: 8 }}>({progress.skippedFiles} {t('nonCodeSkipped')})</span>
                      )}
                    </span>
                  </div>
